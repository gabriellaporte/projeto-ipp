...
<div class="row">
  <div class="col-md-12">
    ...
    <ul class="nav nav-pills flex-column flex-md-row mb-3">
      <li class="nav-item"><a class="nav-link active" href="javascript:void(0);"><i class="bx bx-user me-1"></i> Configurações de Perfil</a></li>
      <li class="nav-item"><a class="nav-link" href="{{ route('account.notifications.settings') }}"><i class="bx bx-bell me-1"></i> Configurações de Notificações</a></li>
    </ul>
    @endif
    <div class="card mb-4">
      ...
      <!-- Account -->
      <form id="formAccountSettings" method="POST" action="{{ $user->id == auth()->user()->id ? route('user.edit') : route('user.edit', $user->id) }}" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <form id="formAddresses" method="POST" action="{{ $user->id == auth()->user()->id ? route('addresses.sync') : route('addresses.sync', $user->id) }}">
          @csrf
              ...
              <div class="input-wrapper row" style="{{ ((!is_null(old('niceName')) && count(old('niceName'))) || count($addresses) > 1) && $loop->first ? 'box-shadow: rgb(9, 3, 104) 0px 24px 3px -24px; margin-bottom: 20px; padding-bottom: 10px;' : ''}}">
                ...
                <div class="mb-3 col-12 d-flex justify-content-end">
                  <a class="destroy-address" href="{{ route('addresses.delete', $address->id) }}">
                    <button type="button" class="btn rounded-pill btn-icon btn-danger remove-btn"><span class="tf-icons bx bx-trash"></span></button>
                  </a>
                </div>
              </div>
            @empty
          ...
          @for($i = 0; $i < count(old('niceName')); $i++)
              <div class="input-wrapper row" style="{{ $i < count(old('niceName')) - 1 ? 'box-shadow: rgb(9, 3, 104) 0px 24px 3px -24px; margin-bottom: 20px; padding-bottom: 10px;' : ''}}">
                <div class="mb-3 col-md-6">
                  <label for="niceName" class="form-label">Apelido <span class="fw-bold text-danger ms-1">*</span></label>
          ...
          <div class="mt-2">
            <button type="submit" class="btn btn-primary me-2">Salvar</button>
            <a href="{{ $user->id == auth()->user()->id ? route('addresses.flush') : route('addresses.flush', $user->id) }}"><button type="button" class="btn btn-outline-danger" >Remover Todos</button></a>
          </div>
        </form>
      </div>
    </div>
